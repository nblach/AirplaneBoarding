...
A main bottleneck for minimizing turn time is the boarding process. Between landing and takeoff, the arriving passengers have to exit the plane, and, more importantly, new passengers need to enter. When entering a plane, passengers need to find their seat, store their hand luggage and sit down. During the this whole process, the passengers can only move in the narrow aisle, oftentimes obstructing each other. Therefore, boarding can take a long time, oftentimes even up to thirty minutes \cite{beus}. Depending on the employed method, however, the interferences between passengers can be reduced, yielding much shorter boarding times. To gain more insight on the key factors influencing the speed and efficiency of boarding, we built a model that can simulate what happens inside an airplane when passengers board.

We built our model based on information from a paper by Van Landeghem and Beuslinck \cite{beus}. In order to verify the validity of our model, we try to reproduce their results, and determine if our model suggests the same relationships between different boarding methods and their effectiveness.

Furthermore, we compare the performance of the boarding methods mentioned by Van Landeghem and Beuslinck and the Steffen method, a new, fast method, proposed by Steffen in 2008 \cite{beus}\cite{steffen}. We simulate boarding, applying these methods on two different modern planes with different loads and measure the total and individual boarding times. Like this, we expect to gain more insight on the most relevant factors for efficient boarding and to verify if the Steffen method is, as expected, faster than the others.

Finally, we investigate the relationship between luggage amount and boarding time. Therefore, we simulate two boarding methods applied for the same airplane with luggage loads ranging from 0\% to 100 \%. We expect to see a super linear increase of boarding time, with increasing luggage load.
...
We only consider airplanes with one door at the front, as all other airplanes with $n \geq 2$ doors can be divided in $n$ sections with one door. Then, boarding can simultaneously be modeled in each section individually, yielding a similar result, if one assumes, that in reality, passengers entering the plane in one section will never go to a different section.
